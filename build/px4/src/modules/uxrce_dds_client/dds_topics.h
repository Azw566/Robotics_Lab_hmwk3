...
#include <uORB/ucdr/actuator_outputs.h>
...
static_assert(sizeof(actuator_outputs_s) <= max_topic_size, "topic too large, increase max_topic_size");
...
struct SendTopicsSubs {
	SendSubscription send_subscriptions[25] = {
			...
			{ ORB_ID(actuator_outputs),
			  uxr_object_id(0, UXR_INVALID_ID),
			  "px4_msgs::msg::dds_::ActuatorOutputs_",
			  "/fmu/out/actuator_outputs",
			  get_message_version<actuator_outputs_s>(),
			  ucdr_topic_size_actuator_outputs(),
			  &ucdr_serialize_actuator_outputs,
			  static_cast<uint64_t>((0 > 0) ? (1e3 / 1000.0) : UXRCE_DEFAULT_POLL_INTERVAL_MS),
			  0
			},
	};
